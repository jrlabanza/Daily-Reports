<?php include "includes/header.php";
    ...
    $_SESSION['report_type'] = 9;

    $status = "";
    $color = "";

    $sql = "SELECT COUNT(*) AS open FROM dmat WHERE stat = 'OPEN' AND isDeleted = 0";
    $result = $connection->query($sql);
    $o = get_data_array($result);
    $sql = "SELECT COUNT(*) AS closed FROM dmat WHERE stat = 'CLOSED' AND isDeleted = 0";
    $result = $connection->query($sql);
...
<div class="card" style="margin-left:10px; margin-right:10px; margin-top:10px;">
    <div class="card-header" style="font-size:15px;">EQUIPMENT ENGINEERING DAILY MEETING
    ...
    <div class="card-body">
            ...
            <?php if (empty($_POST['startDate'])  && empty($_POST['endDate'])){     
                ...
                $showallrec_query ="SELECT * FROM dmat WHERE isDeleted = 0 ORDER BY entry_date DESC";
                ...
                $newquery ="SELECT * FROM dmat WHERE (DATE(entry_date) BETWEEN('$startDate' and '$endDate')) AND isDeleted = 0 order by entry_date DESC";
