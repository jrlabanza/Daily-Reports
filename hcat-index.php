<?php include "includes/header.php";
    ...
    $sql = "SELECT COUNT(*) AS open FROM hcat WHERE stat = 'OPEN' AND isDeleted = 0";
    $result = $connection->query($sql);
    $o = get_data_array($result);
    $sql = "SELECT COUNT(*) AS closed FROM hcat WHERE stat = 'CLOSED' AND isDeleted = 0";
    $result = $connection->query($sql);
...
<div class="card" style="margin-left:10px; margin-right:10px; margin-top:10px;">
    <div class="card-header" style="font-size:15px;">HARDWARE C-SHOP ACTION TRACKER
    ...
    <div class="card-body">
            ...
            <?php if (empty($_POST['startDate'])  && empty($_POST['endDate'])){     
                ...
                $showallrec_query ="SELECT * FROM hcat WHERE isDeleted = 0 ORDER BY entry_date DESC";
                ...
                $newquery ="SELECT * FROM hcat WHERE (DATE(entry_date) BETWEEN('$startDate' and '$endDate')) AND isDeleted = 0 order by entry_date DESC";
